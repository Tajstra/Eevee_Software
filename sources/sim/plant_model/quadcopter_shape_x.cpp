...
namespace sim
{
    QuadcopterShapeX::QuadcopterShapeX(Time::s const sample_time) : _angular_rates(sample_time), _angular_displacement(sample_time)
    {
    }
    ...
    QuadcopterShapeX::updateAngularRates(Eigen::Vector4f const &input_signals)
    {
        ...
        std::transform(propeller_omegas.begin(), propeller_omegas.end(), propeller_omegas_squared.begin(), [](float x)
                       { return x * x; });
        ...
        float const torque_inertia_x = _angular_rates.getValue()[Q] * _angular_rates.getValue()[R] * (_parameters.inertia_y - _parameters.inertia_z);
        float const torque_inertia_y = _angular_rates.getValue()[P] * _angular_rates.getValue()[R] * (_parameters.inertia_z - _parameters.inertia_x);
        float const torque_inertia_z = _angular_rates.getValue()[P] * _angular_rates.getValue()[Q] * (_parameters.inertia_x - _parameters.inertia_y);
